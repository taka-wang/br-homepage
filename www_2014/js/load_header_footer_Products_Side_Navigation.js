var load_header_footer_Side_Navigation = function() {
    ...
    function Set_Side_Navigation(json_data) {               
        var parse_Side_navigation_data  = {"main_title":"", series_data:[]};   //save parse Data waiting to be sent to Handlebars compiler

        parse_Side_navigation_data.main_title = json_data.Products[(language_type == "English") ? "title" : "tw_title"];

        $.each(json_data.Products.Sub_item, function (index, Sub_item_data) {
            $.each(Sub_item_data, function (Products_series, Products_series_obj) {
                var HTML_info_Products_series = Products_series_obj[(language_type == "English") ? "title" : "tw_title"];
                var HTML_info_Products_link   = Products_series_obj.link;

                if (Products_series_obj.title === "Accessory" || Products_series_obj.title === "Field Perfect" ) {
                    var products_data = [];

                    $.each(Products_series_obj.data, function (index, Products) {

                        products_data.push({"link" : Products.link, "title" : Products[(language_type == "English") ? "title" : "tw_title"]});

                    });

                    // if add_series_link is true, handlebars will add href into the <h3> like this "<h3><a href="{{Products_series_link}}">{{Products_series}}</a></h3>".
                    parse_Side_navigation_data.series_data.push({"Products_series" : HTML_info_Products_series, "Products_series_link" : HTML_info_Products_link ,"products_data": products_data, "add_series_link" : 'true' });

                } else if (Products_series_obj.title !== "Accessory") {
                    var products_data = [];
                    ...
                    parse_Side_navigation_data.series_data.push({"Products_series" : HTML_info_Products_series, "Products_series_link" : HTML_info_Products_link ,"products_data": products_data });
                } else {
                    console.log("pares Side_Navigation data error.");
                }
        ...
        var html = Handlebars.templates['products_side_navigation_template'](parse_Side_navigation_data);   
    ...
    function Set_header() {
        ...
        //After pre-compile get the pre-compile Handlebars templates(js/templates/header_template.js)
            var html = Handlebars.templates['products_main_page_header_template'](parse_header_data);
            $('#load_header').append(html);
    ...
    function Set_footer() {
        ...
        var html = Handlebars.templates['products_main_page_footer_template'](parse_footer_data); 
...
}();//End load_header_footer_Products_Side_Navigation 